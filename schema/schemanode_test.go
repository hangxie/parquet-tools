...
func TestConvertInt96ToTimestamp(t *testing.T) {
	testCases := []struct {
		name             string
		uri              string
		expectedFields   []string
		totalINT96Fields int
	}{
		{
			name:             "all types file",
			uri:              "../testdata/all-types.parquet",
			expectedFields:   []string{"Int96"},
			totalINT96Fields: 1,
		},
		{
			name:             "int96 specific file",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Find INT96 fields before conversion to verify they exist
			pathMapBefore := schemaRoot.GetPathMap()
			foundCount := 0
			for _, node := range pathMapBefore {
				if node.Type != nil && *node.Type == parquet.Type_INT96 {
					foundCount++
				}
			}
			require.Equal(t, tc.totalINT96Fields, foundCount)

			// Perform conversion
			convertedFields := schemaRoot.ConvertInt96ToTimestamp()

			// Verify returned map
			require.Len(t, convertedFields, tc.totalINT96Fields)
			for _, field := range tc.expectedFields {
				require.Contains(t, convertedFields, field)
			}

			// Verify conversion in the tree
			pathMapAfter := schemaRoot.GetPathMap()
			for path, node := range pathMapAfter {
				isConvertedField := false
				for _, f := range tc.expectedFields {
					if node.Name == f {
						isConvertedField = true
						break
					}
				}

				if isConvertedField {
					// Field should now be INT64
					require.NotNil(t, node.Type, "Path: %s", path)
					require.Equal(t, parquet.Type_INT64, *node.Type, "Path: %s", path)

					// Logical type should be TIMESTAMP(NANOS, true)
					require.NotNil(t, node.LogicalType, "Path: %s", path)
					require.True(t, node.LogicalType.IsSetTIMESTAMP(), "Path: %s", path)
					require.True(t, node.LogicalType.TIMESTAMP.IsAdjustedToUTC, "Path: %s", path)
					require.NotNil(t, node.LogicalType.TIMESTAMP.Unit, "Path: %s", path)
					require.True(t, node.LogicalType.TIMESTAMP.Unit.IsSetNANOS(), "Path: %s", path)

					// Converted type should be nil
					require.Nil(t, node.ConvertedType, "Path: %s", path)

					// Encoding should be PLAIN
					require.Equal(t, "PLAIN", node.Encoding, "Path: %s", path)
				} else if node.Type != nil {
					// Other fields should not be INT96 anymore (if they were)
					require.NotEqual(t, parquet.Type_INT96, *node.Type, "Path: %s", path)
				}
			}
		})
	}
