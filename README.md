...
Cloudflare R2, and LocalStack can be configured with the standard AWS SDK
...
`parquet-tools` accepts all common spellings of an Azure blob URI, following the [HDFS URL format](https://docs.microsoft.com/en-us/azure/hdinsight/hdinsight-hadoop-use-blob-storage#access-files-from-within-cluster):
* one of these schemes:
  * `wasbs://`, the blob-style scheme used by HDFS, with `wasb://` as its alias
  * `abfss://`, the filesystem-style scheme used by ADLS Gen2, Databricks, Synapse, and Fabric, with `abfs://` as its alias
  * `az://`, the scheme used by [adlfs](https://github.com/fsspec/adlfs)
...
The host can carry either the blob endpoint suffix `.blob.core.windows.net` or the DFS endpoint suffix `.dfs.core.windows.net`: ADLS Gen2 serves the same data on both, and `parquet-tools` translates a DFS host to the blob host that the Azure SDK talks to. These URIs all refer to the same blob:
...
`az://` additionally accepts a shorthand form that omits the storage account, which then has to come from the `AZURE_STORAGE_ACCOUNT_NAME` environment variable:
